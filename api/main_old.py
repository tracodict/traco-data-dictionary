...
async def search_all(
    ...
    Examples:
    - `/api/gateway/proxy/dict/search?query=Order` - Find all entities containing "Order"
    - `/api/gateway/proxy/dict/search?query=^Order&is_regex=true` - Find entities starting with "Order"
    - `/api/gateway/proxy/dict/search?query=Limit&search_type=field` - Find fields containing "Limit"
    """
    if not fix_parser:
...
# Message endpoints
@app.get("/api/gateway/proxy/dict/messages", response_model=List[MessageSummary])
...
# Field endpoints
@app.get("/api/gateway/proxy/dict/fields", response_model=List[FieldSummary])
...
@app.get("/api/gateway/proxy/dict/fields/{tag}", response_model=FieldDetail)
...
@app.get("/api/gateway/proxy/dict/fields/name/{name}", response_model=FieldDetail)
...
# Component endpoints
@app.get("/api/gateway/proxy/dict/components", response_model=List[ComponentSummary])
...
@app.get("/api/gateway/proxy/dict/components/{name}", response_model=ComponentDetail)
...
@app.get("/api/gateway/proxy/dict/codesets", response_model=List[CodeSetSummary])
...
@app.get("/api/gateway/proxy/dict/codesets/{tag}", response_model=List[Enum])
...
# Utility endpoints
@app.get("/api/gateway/proxy/dict/versions", response_model=List[str])
async def get_versions():
    """Get list of available FIX versions"""
    return [version.value for version in FIXVersion]

@app.get("/api/gateway/proxy/dict/sections", response_model=List[str])
async def get_sections():
    """Get list of available section IDs"""
    return [section.value for section in SectionID]

@app.get("/api/gateway/proxy/dict/categories", response_model=List[str])
...
@app.get("/api/gateway/proxy/dict/{version}/msg/{component_id}", response_model=MessageDetail)
...
@app.get("/api/gateway/proxy/dict/{version}/tag/{tag}", response_model=FieldDetail)
...
@app.get("/api/gateway/proxy/dict/{version}/cmp/{component_id}", response_model=ComponentDetail)
