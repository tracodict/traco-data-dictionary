<script lang="ts">
  ...
  import type { MessageSummary, IServerSideGetRowsRequest, LoadSuccessParams } from '../lib/api-client';
    ...
    serverSideDatasource = {
      getRows: async (params) => {
        ...
        try {
          ...
          console.error('MessageList SSRM error:', error);
          params.fail();
        } finally {
          loading = false;
        }
  ...
  function handleRowSelection(event: any) {
    ...
    if (selectedRows.length > 0) {
      console.log('Selected message:', selectedRows[0]);
      // TODO: Implement message detail view or navigation
    }
  }
</script>
